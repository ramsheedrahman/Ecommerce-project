...
const CategoryManager = () => {
  ...
  const handleCreateCategory = async (e) => {
    ...
    try {
      ...
      const res = await axios.post("http://localhost:8000/category/create-category", { name }, {
        headers: {
          Authorization: token
        }
      });
  ...
  const handleUpdateCategory = async (e) => {
    ...
    e.preventDefault();
    try {
      const res = await axios.put(`http://localhost:8000/category/update-category/${selected._id}`, { name:newCategoryName },{
        headers: {
          Authorization: token
        }
      });
...
const confirmDelete=async(e)=>{
  ...
  try {
    const token = JSON.parse(localStorage.getItem('auth')).token;
    const res=await axios.delete(`http://localhost:8000/category/delete-category/${selected._id}`,{
      headers: {
        Authorization: token
      }
    })
