...
function Orders() {
  ...
  const getOrders = async () => {
    try {
      const { data } = await axios.get('http://localhost:8000/product/get-order-products', {
        headers: {
  ...
  const invoiceDownload = async (id) => {
    try {
      const response = await axios.get(`http://localhost:8000/product/download-invoice/${id}`, {
  ...
  const orderCancelhandler = async (id) => {
    try {
      const { data } = await axios.put(`http://localhost:8000/product/cancel-order/${id}`, { status: cancellStatus }, {
        headers: {
          Authorization: User.token,
        },
      });
            ...
            {orders.map((order) => (
              <div key={order._id} className="order-details-card bg-light">
                ...
                <div className="order-actions">
                  ...
                  <button className="btn btn-download" onClick={()=>invoiceDownload(order.orderID)}>Download Invoice</button>
